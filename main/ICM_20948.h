...
protected:
    ...
    double getGyrDPS(int16_t axis_val);
    double getAccMG(int16_t axis_val);
...
public:
    ...
    double magX(void); // micro teslas
    double magY(void); // micro teslas
    double magZ(void); // micro teslas

    double accX(void); // milli g's
    double accY(void); // milli g's
    double accZ(void); // milli g's

    double gyrX(void); // degrees per second
    double gyrY(void); // degrees per second
    double gyrZ(void); // degrees per second

    double temp(void); // degrees celsius

    double deltat(void); // degrees celsius
    ...
    // interval time [sec]
    double deltat_sec = 0.0;

    double pitch, yaw, roll;
    double q0, qx, qy, qz;
    bool is_quaternion_updated = false;
};
